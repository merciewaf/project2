...
require_once("db.php");
if(count($_POST)>0) {
	$sql = "UPDATE guestlink set feedback='" . $_POST["feedback"] . "', status='Completed', feedback_time=NOW() WHERE gl_id='" . $_GET["gl_id"] . "'";
...
<td><select name="feedback" class="form-control">
<option value="pick">Select Option</option>
<?php
//require_once("db.php");
// Check connection

$sql = "SELECT * FROM gl_feedback";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
    	...
    	echo "<option value='".$row['feedback']."'>".$row['feedback']."</option>";
...
<tr>
<td><input type="hidden" name="feedback_time"  /></td></tr>
<tr>
